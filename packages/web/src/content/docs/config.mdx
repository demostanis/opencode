...
Set the optional top-level `shell` option to select the executable used for commands invoked by the agent `bash` tool. An explicitly configured value is used as-is.
...
When omitted, OpenCode uses `$SHELL` unless it points to Fish or Nushell, then uses its platform fallback.

This option does not change interactive TUI shell mode.
